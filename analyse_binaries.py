...
# load the orbit file
df_orb=bf.load_orb_file("test_run_0_orbit_search_faster_hel.txt")
...
while file_iter<10:
    try:
        t,df_dat=bf.load_dat_file("{}/{}/dat{:07d}_{}.txt".format(run_path,run_dir,n_file,file_iter))
        break
    except:
        file_iter+=1
...
# df_cut.to_csv("dat_cut_{:07d}_0.txt".format(n_file))
# t=bf.read_dat_file_time("dat{:07d}_0.txt".format(n_file))
# df_cut=pd.read_csv("dat_cut_{:07d}_0.txt".format(n_file),index_col=0)
...
# plot all particles, where colour/size scales with mass
